...
def process_csv_jf_to_awin(content: bytes):
    ...
    for row in reader:
        ...
        if status == "R":
            ...
            if (row[11]==""):
                amendedRow = [row[0], row[4], "AMENDED", "Teilstorno", row[5][:-2], "", "EUR"]
            else:
                amendedRow = [row[0], row[4], "AMENDED", row[11], row[5][:-2], "", "EUR"]
...
def process_csv_jf_bonus(content: bytes):
    bonus = [
        ["Advertiser_ID", "Publisher_ID", "Verguetungs_art", "Sale_betrag", "Bonus/Provisions_betrag", "Provisions_status", "Klick_Ref", "Auftrags_Nr"]
    ]
    ...
    for map_publisher_id, map_bonus in bonus_map.items():
        if map_bonus >= 10:
            bonus.append(["14899", map_publisher_id, "bonus", "0", bonus_to_cash(map_bonus), "confirmed", "", "BONUS_" + str(month) + "_" + str(year) + "_" + str(map_bonus) + "_" + str(map_publisher_id)])
