...
async def upload_file(file: UploadFile = File(...), option: str = Form(...)):
    ...
    match option:
        case "jf_to_awin":
            print(option)
            accepted_data, declined_data, amended_data = (
                editExcel.process_csv_jf_to_awin(content=content)
            )
            ...
            with zipfile.ZipFile(
                ...
                zip_file.writestr(
                    f"accepted_{today.strftime('%Y%m%d')}_{file.filename}.csv",
                    csv_to_str(accepted_data),
                )
                zip_file.writestr(
                    f"declined_{today.strftime('%Y%m%d')}_{file.filename}.csv",
                    csv_to_str(declined_data),
                )
                zip_file.writestr(
                    f"amended_{today.strftime('%Y%m%d')}_{file.filename}.csv",
                    csv_to_str(amended_data),
                )
